...
The inspector renders inside a WPF `Popup` primitive (`PlacementMode.MousePoint` near the cursor, `StaysOpen=False` for click-outside dismissal, `AllowsTransparency=True` for the rounded-corner border treatment). The Popup hosts a `UserControl` (`LockInspectorPopover`) whose visual surface is a `Border` matching the existing C6a viewer-error banner style at `DocumentDetailView.xaml:138-151` — `BrushSurface2` background, `BrushBorder` 1px, 6px corner radius. The header reads "Why is this locked?" using the existing `TextStyleSubheading` token. The body is an `ItemsControl` over the chain links; each link card renders the `Tag` chip in purple (per SPEC §4.4 — purple is the cross-cutting linkage accent, the right reuse here), the `Detail` text, and (when `NavigationEntityType` is non-null) a navigation indicator (live click-through deferred to a later commit when an `INavigationService` lands). Between non-terminal rows, the `Because` connector renders in muted text (`BrushTextDim`) as `↳ because <Because>`. The terminal row gets a small "root cause" affordance.

An earlier C7b draft hosted the popover in a chromeless top-level `Window` with auto-close on `Deactivated`. That shape interacts poorly with WPF activation when called after an `await`: the synchronous user-input context that allows focus-stealing has cleared by the time `Show()` runs, the OS refuses to activate the chromeless Window, and `Deactivated` fires immediately on Show — the popover closes before the user sees it. The Popup primitive sidesteps this: it does not steal focus, click-outside dismissal is handled by `StaysOpen=False`'s internal mouse-capture hook, and positioning works regardless of activation state. C7b's first smoke-walk surfaced the activation-after-await failure; the popup-primitive refactor is the corrective amendment.
...
- **`LockInspectorPopover` (UserControl hosted in a `Popup` primitive) + `LockInspectorViewModel` + `ILockInspectorPrompter`** ship in C7b, not C7a. C7a is service-layer-only.
